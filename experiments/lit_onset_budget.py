"""experiments/lit_onset_budget.py -- A CONTROL, not an anchor. Its onset model counts tau_T twice.

This is the carrier x SNR sweep run ON TOP of experiments/literature_onset_test.py's lag onset, so
it inherits that script's defect in full: the command is pre-lagged by tau = 0.2 s and then driven
through an airframe whose own closed-loop response already IS 0.179 s, inflating the onset interval
2.35x (48.45 -> 113.8 ms). See literature_onset_test.py's header and
experiments/onset_model_equivalence.py for the measurement. Its headline cell -- Ka-band/40 dB,
+58.0 ms, 91% detection -- was drafted as the letter's new anchor and must not be used as one.

The CARRIER axis it sweeps is still sound and still interesting; it is only the onset model
underneath that is wrong. Nothing in the manuscript cites this artifact.

(The module docstring below was copied from slow_cue_budget.py and still carries that file's name
and its 15 s framing; it describes the carrier/SNR machinery, which is shared and correct.)
--------------------------------------------------------------------------------------------------
...
TAU = 0.20                          # fan2016 tau_T -- the literature onset model
