"""experiments/lit_onset_budget.py -- carrier x SNR sweep under a first-order lag onset.

The command is a first-order lag onset with the literature time constant tau_T = 0.20 s [fan2016],

    a(t) = A (1 - exp(-(t - t_pre) / tau_T)),   t >= t_pre

and the lagged command then drives the airframe model. The airframe's closed-loop step response
already has a time constant of 0.179 s, so this cascade applies the lag twice and lengthens the
onset interval 2.35x, from 48.45 ms to 113.8 ms; experiments/onset_model_equivalence.py measures
the cascade. The paper does not use this artifact.

The cue is z = trailing mean |dphi/dt|^2 with phi = (4*pi*r/lambda) sin(delta), so

    z_signal  ~  (r/lambda)^2 * (A/T)^2        A = fin excursion, T = time scale of the fin motion
    z_noise   ~  set by the receiver, independent of T

and a shorter carrier raises the signal term as 1/lambda^2:
...
The script sweeps the X, Ka and W bands at 40-70 dB SNR on the supersonic-cruise trajectories. A
cell is detectable when detection >= 50% at a no-cue null false-alarm rate < 5%, the bar used for
the HGV sweep, and converting when it is also detectable with a positive median paired advantage
over CUSUM.

    python experiments/lit_onset_budget.py --json runs/ml/lit_onset_budget.json
"""
import argparse
import json
import os
...
TAU = 0.20                          # first-order lag time constant tau_T [fan2016], s
BANDS = (("X", 10.0e9), ("Ka", 35.0e9), ("W", 95.0e9))
SNRS = (40.0, 50.0, 60.0, 70.0)
...
def run_cell(cell):
    ...
    def synth_lag(shape, dur_s, amp, t_pre, t_post):
        target = 3.0 * ONSET_G_MS2
        if amp > target:
            t_hit = -TAU * np.log(max(1.0 - target / amp, 1e-9))
            t_post = float(np.clip(2.0 * t_hit, t_post, 3.0))
        n = int((t_pre + t_post) / ml.DT_FINE)
        tt = np.arange(n) * ml.DT_FINE
        a = np.zeros(n)
        on = tt >= t_pre
        a[on] = amp * (1.0 - np.exp(-(tt[on] - t_pre) / TAU))
        return tt, np.abs(a), t_pre

    ml.synth_command = synth_lag
    ...
    return dict(band=band, fc_hz=fc, snr_db=snr, tau_s=TAU, n=int(a.size),
...
def main():
    ...
    print("LITERATURE-ONSET LINK BUDGET -- carrier x SNR at the literature tau_T = %.2f s" % TAU)
    print("  the reported point is X-band / 40 dB, where the cue does not alarm under this onset.")
    ...
    # Detectable: the cue clears the no-cue null. Converting: it is detectable and also beats the
    # comparator. Carrier and SNR set detectability; alarm timing decides the advantage.
    ...
    if seen and not won:
        ...
        print("   The muD arm still alarms later than the")
        ...
        print("   The muD lead comes from fast transients. A state-based detector integrates a slow")
        print("   onset well, so higher SNR makes the cue detectable without making it earlier than")
        print("   the comparator.")
    elif won:
        print("\n=> A realisable operating point both detects and beats the comparator.")
    else:
        print("\n=> No cell is detectable across 40-70 dB SNR and a 10x wavelength reduction.")
    if args.json:
        os.makedirs(os.path.dirname(args.json), exist_ok=True)
        json.dump(dict(tau_s=TAU, sigma=SIGMA, seeds=SEEDS, reps=REPS, amp_factor=AMP,
