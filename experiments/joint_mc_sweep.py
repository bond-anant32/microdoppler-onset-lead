"""experiments/joint_mc_sweep.py -- joint Monte Carlo over all free parameters.

sensitivity_sweep.py varies one parameter at a time (OAT) and finds a positive sign in all 62 cells
where every trajectory pairs. OAT does not sample interactions, and the parameters are coupled:
CN_delta sets both the direct fin lift seen by the kinematic arm and part of the cue, and servo
bandwidth sets both delta(t) and a_z(t). This script draws every free parameter independently on
each trial, so combinations such as low CN_delta, diluted RCS, low SNR and a non-step command are
sampled together.

Parameters drawn jointly (ranges are the OAT sweep's endpoints):
    sigma        log-uniform 0.03 .. 3.0 m/s^2   (above the measured crossover; below it the paper
                                                  reports the kinematic comparator leading)
    snr_db       uniform 20 .. 50 dB
    CN_delta     uniform 0.05 .. 1.20
    wn_servo     log-uniform 25 .. 300 rad/s     (5 and 10 rad/s are excluded: in the OAT sweep only
                                                  4 and 19 of 30 trajectories pair, so draws at
                                                  those values would score a selected subset)
    rate_max_dps uniform 50 .. 800 deg/s
    amp_factor   uniform 0.2128 .. 0.4931        (spans the OAT feasibility readings 0.2128, 0.2798,
                                                  0.4931)
    rho          uniform 0.05 .. 1.0             (fin's share of return power, the dilution parameter)
    shape        {step, raised-cosine, sinusoid}  (commanded acceleration shape)

Output: the fraction of trials with a positive median advantage, the magnitude distribution, and
each trial where the sign reverses, listed with its parameter values.

A trial enters the pooled statistics only if all 30 trajectories pair, as in the OAT sweep.
Partially paired trials are listed separately, since whether a trajectory pairs is correlated with
the outcome.
...
def main():
    ...
    print("Joint Monte Carlo: all free parameters drawn together on each trial")
    ...
    if neg:
        print("\nSign reversals in the joint draws (%d):" % len(neg))
        for r in neg:
