"""Alternative muD detector families on a slow sinusoidal command (period T = 15 s).

Three statistic families (band power at the command frequency, a filter matched to the command-shape
derivative, and unwrapped phase deviation from the cruise baseline) are evaluated at several dwells,
with and without CUSUM accumulation. Each variant is raced against CUSUM on noisy lateral acceleration
(sigma = 0.3 m/s^2) at 40 dB, with a no-cue null for the false-alarm rate.

    python experiments/slow_cue_detector_alt.py --json runs/ml/slow_cue_alt.json
"""
import argparse
import json
import os
...
# Statistics for the alternative detector families.

def stat_band_power(t, s, dwell_s, period_s):
    """Family 1: band power at the weave frequency 1/T (band 0.8-1.2 x 1/T); "unformable" if the band cannot be formed."""
...
def stat_matched_filter(t, s, dwell_s, period_s):
    """Family 2: dphi filtered with a unit-norm |cos(2 pi t / T)| template one dwell long (command-shape derivative)."""
    import experiments.dphi_sweep as ds
    dphi_val = ds.dphi(s)
    dt_r = ds.DT_R
...
def stat_phase_deviation(t, s, dwell_s):
    """Family 3: magnitude of the unwrapped phase deviation from its cruise mean, averaged over one dwell."""
    import experiments.dphi_sweep as ds
...
def run_cell(arg):
    ...
    # per-trajectory medians of advantage, muD lead and CUSUM lead; per-repetition detection and false alarm
...
def main():
    ...
    print("Alternative detector families on a slow sinusoidal command (T = 15 s)")
