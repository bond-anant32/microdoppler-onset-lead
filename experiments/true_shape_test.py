"""experiments/true_shape_test.py -- supersonic_cruise advantage under the generator's command shape.

experiments/multiclass_lead.SHAPES maps

    "supersonic_cruise": ("step", 0.0)

and trajectory_generators/maneuvering.py commands the sinusoid

    a_lat_command(t) = weave_lat_accel * sin(2*pi*weave_cycles*m),  m = (t-weave_start_t)/weave_dur

with weave_dur = clip((1-weave_start_frac)*duration, 20, 60) s and weave_cycles sampled at 3.5-4.9
by generate_dataset. The half-period at onset is several seconds. A step has infinite initial
slope, the most favourable input for a statistic that scales as the square of fin rate. At a
0.30 s command duration, step, raised-cosine and sinusoid shapes give +17.0, +44.5 and +42.5 ms
(experiments/audit_t10.py, M2).

This script measures the operating point under the generator's shape and period:

    A  step                   -- control, reproduces +17.0 ms
    B  sinusoid, per-seed period from that seed's weave_cycles and weave_dur
    C  sinusoid at a fixed 36 s period
...
def weave_period_s(seed):
    """Weave period (s) for `seed`, weave_dur / weave_cycles, from generate_dataset._sample_params."""
    ...
    # synth_command's `dur_s` is the full period of one sine; the generator fits `cycles` periods
...
def run(mode):
    ...
    for sd in range(SEEDS):
        ...
        else:                                            # sinusoid at a fixed 36 s period
...
def main():
    ...
    print("Command shape test: step control and the generator's sinusoidal weave")
    ...
    print("-" * 88)
    for r in rows:
        lab = {"step": "A step (reported result)",
    ...
    print("\nControl: the step row reproduces the reported +17.0 ms -> %s"
