"""experiments/slow_cue_budget.py -- carrier frequency and SNR sweep at a slow command period.
...
A 15 s weave moves the fin ~60x slower than a step, which costs ~35 dB of statistic. At X-band
(lambda = 3.0 cm) and 40 dB SNR the detector does not alarm at this period (shape_period_sweep.py),
and none of the six statistics in slow_cue_detector.py reaches the detection bar defined below.

Carrier wavelength and SNR are radar design parameters, and the 1/lambda^2 term converts a shorter
carrier directly into sensitivity:

    band    lambda     gain vs X-band
    X        3.00 cm      0 dB   (default)
    Ku       2.00 cm    +3.5 dB
    Ka       0.86 cm   +10.9 dB
    W        0.31 cm   +19.7 dB

The script sweeps carrier (X, Ka, W) and SNR (40-85 dB) at the generator's 15 s command period.
A cell is detectable when detection >= 50% at a no-cue null false-alarm rate < 5%, the same bar
applied to the boost-glide exclusion. A detectable cell is converting when its median advantage
over the kinematic comparator is also positive.
...
PERIOD = 15.0                       # generator weave period for supersonic_cruise (s)
...
def run_cell(cell):
    ...
    # the carrier enters only through lambda = C/FC; patch every module binding that reads it
    cdt.FC_HZ = fc
    dps.FC_HZ = fc
...
def main():
    ...
    print("  the default operating point is X-band / 40 dB, where the detector does not alarm.")
    ...
    # Detectable: the cue clears the null bar. Converting: detectable and the median advantage
    # over the comparator is positive.
    ...
    if seen and not won:
        ...
        print("   A link budget does not buy the advantage. At this command period the muD arm alarms later than the")
        ...
        print("   The muD lead is a fast-transient effect. A state-based detector integrates a slow")
        print("   manoeuvre well, so added SNR makes the cue detectable and leaves it later than the")
        print("   comparator.")
    elif won:
        print("\n=> A realisable operating point both detects and beats the comparator.")
    else:
        print("\n=> No cell is detectable within a 45 dB SNR sweep and a 10x wavelength reduction.")
    if args.json:
