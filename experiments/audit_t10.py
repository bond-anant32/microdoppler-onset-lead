"""Stability checks on the muD-over-CUSUM advantage at the paper's operating point (supersonic cruise,
40 dB, 2 ms dwell, kinematic noise sigma 0.3 m/s^2).

  H4   median advantage in three other seed blocks against the CI of the published block (90000)
  H5   CI endpoints as a function of the bootstrap resample count
  M2   advantage, detection rate and command-to-onset budget for step, raised-cosine and sinusoid
       command shapes
  M10  r^2 of the budget regressed on commanded amplitude, altitude and Mach, the three scalars each
       trajectory draw is built from

The command amplitude factor defaults to 0.2798 (1/3.57), the derived aerodynamic-feasibility
amplitude used in the paper; --amp-factor overrides it.
...
def main():
    ...
    print("=== H4: other seed blocks against the published CI ===")
    ...
    print("\n=== H5: CI endpoints against bootstrap resample count ===")
    h5 = {}
    ...
    print("\n=== M10: budget regressed on amplitude, altitude and Mach ===")
    if bud0.size > 3:
        ...
        print("     (amplitude, altitude, Mach) triples.")
