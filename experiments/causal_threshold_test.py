"""experiments/causal_threshold_test.py -- lead advantage under anchored and causal thresholds.

The reported zero-false-alarm threshold is

    eta = max_{t in W} z(t),   W = [t_on - 0.60, t_on - 0.12] s

W is anchored to t_on, the onset being timed, and no sample inside W can exceed eta. An operational
tracker does not know t_on. This script applies three threshold rules, identically to the muD
statistic and the CUSUM comparator, and computes the paired advantage under each:

  A  anchored       eta from [t_on-0.60, t_on-0.12]; the script checks that it gives the reported
                    +17.0 ms.
  B  rolling causal eta(t) = max z over the trailing window [t-0.60, t-0.12], recomputed at every
                    decision sample with no knowledge of t_on. This is trailing self-calibration
                    behind a guard band, which a tracker can run online.
  C  anchor error   eta from [t_on+D-0.60, t_on+D-0.12] with D = +50, +100, +200 ms. The window
                    extends past the command and takes in manoeuvre samples, which models rule B
                    with a guard band that is too short.

The script prints the change in median advantage from rule A to rule B and labels the anchoring
bookkeeping when the change is at most 2 ms and load-bearing otherwise.

causal_lead searches for an alarm in [t_on - search_pre, t_on + 0.30]; search_pre is swept in
sensitivity_sweep.py. The pre column counts muD alarms that precede the command under each rule.
...
CRUISE, GUARD = 0.60, 0.12                     # calibration window [t - CRUISE, t - GUARD], s
MODES = [("shipped", 0.0), ("rolling", 0.0),
         ("anchor", 0.05), ("anchor", 0.10), ("anchor", 0.20)]


def rolling_thr(t, stat, lo=CRUISE, hi=GUARD):
    """eta(t) = max of stat over the trailing window [t-lo, t-hi], computed without t_on.

    O(n) via a sliding-window maximum. Returns an array the length of t. Until the window lies
    wholly inside the record the threshold is +inf, so no alarm is raised while the calibration
    buffer fills.
    """
    n = len(t)
    dt = float(t[1] - t[0])
...
def shifted_thr(t, stat, t_on, shift):
    """Anchored threshold with its window moved `shift` seconds later; +inf if under 9 samples."""
...
def lead_vs_threshold(t, stat, thr, t_on, need_ms=3.0, search_pre=0.25):
    """causal_lead with `thr` given as a scalar or a per-sample array.

    Decimation, search window and the 3-sample run rule match causal_dwell_test.causal_lead, so the
    rules differ only in the threshold. Returns the lead in seconds, or None if no alarm is found.
    """
...
def main():
    ...
    print("CAUSAL-THRESHOLD TEST -- anchored, rolling and shifted threshold rules")
    ...
    for sd in range(SEEDS):
        ...
        for w in wins:
            ...
            for r in range(REPS):
                # statistics are computed once per realisation; only the threshold rule varies
    ...
    print("\nControl: rule A reproduces the reported +17.0 ms -> %s"
    ...
    if ctrl is not None and roll is not None:
        ...
        print("Result: %s"
              % ("a trailing self-calibration reproduces the result"
                 if abs(d) <= 2.0 else "the result depends on knowing t_on"))
    if args.json:
