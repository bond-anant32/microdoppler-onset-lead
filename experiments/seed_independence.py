"""experiments/seed_independence.py -- sensitivity of the operating point to the shared noise block.

measure() in experiments/multiclass_lead.py seeds the micro-Doppler arm at 4000 + r and the
comparator at 4991 + r, where r is the repetition index. The trajectory index does not enter
either seed, so all 30 trajectories are measured on the same noise realizations (common random
numbers). This reduces the variance of the paired comparison, and it makes the bootstrap interval
and signed-rank test over the 30 per-trajectory values conditional on that one noise block.

Each cell re-runs the operating point with a per-trajectory offset added to both seeds:

  * within a trajectory the two arms stay on the same realization index, so the pairing for the
    signed-rank test is preserved;
  * across trajectories the noise streams are disjoint.

Offset 0 is the control and reproduces the sigma = 0.3 row of runs/ml/sigma_sweep_derived.json.

measure() takes no seed argument, so this script wraps np.random.default_rng and remaps only the
two seed ranges measure() draws from. class_windows keeps 90000 + sd, so the trajectories are
those of the sigma sweep, and the bootstrap keeps its own seed.
...
def _patched(seed=None, *a, **k):
    """Remap only the two seed ranges measure() draws its noise from."""
...
def main():
    ...
    print("  offset 0 is the control and reproduces the sigma=0.3 row of the sigma sweep.\n")
    ...
    print("\n  reported %+.2f ms | %d independent blocks: median %+.2f, range %+.2f to %+.2f, "
          "all positive %s"
          % (out["shipped"], out["independent_n"], out["independent_median"],
             out["independent_lo"], out["independent_hi"], out["all_positive"]))
    print("\n  The offset-0 bootstrap interval is formed over trajectories within one block and")
    print("  excludes the between-block spread shown above.")
