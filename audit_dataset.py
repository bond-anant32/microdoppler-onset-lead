"""
audit_dataset.py - physical-envelope checks for generated trajectories.

audit_trajectory() checks one trajectory (positions Nx3 ECEF, dt) against the envelope of its
class in CLASS_SPECS:
  ...
  - a=v^2/r:       peak lateral load <= class cap and min turn radius >= class floor
                   (no impossibly sharp turns)
  - no-impulse:    bounded jerk (raised-cosine easing gives continuous acceleration; a spike
                   indicates a discontinuity)

experiments/generate_dataset.py and experiments/class_profiles.py call audit_trajectory() on each
generated trajectory. Run as a script, this module checks the four force-integrated generators and
...
# Per-class plausibility envelopes (open-source calibrated; lenient, so only unphysical samples
# fail). g = lateral load factor; radius in km.
CLASS_SPECS: Dict[str, dict] = {
    # jerk_cap 2000: a long-range ballistic RV reenters at ~7 km/s, and the drag-deceleration onset at 0-10 km
    # altitude is a brief physical high-jerk event (measured peak ~1500 m/s^3, 0.3% of steps, all below 10 km).
    # The cap sits above the slower MaRV reentry (1000) and below the MIRV reentry (5000).
    ...
    # sub-ballistic but lofts to ~500-650 km, below the ~1000 km apogee of a ballistic shot of that range and
    # above the ~400 km of a shorter-range MaRV. jerk_cap: reentry drag onset and the bounded terminal pull-up
    # are physical high-jerk events (cf. the mirv/decoy reentry jerk allowance).
    "marv":       dict(alt_max_km=700.0,  spd_min=250.0, spd_max=8000.0, g_cap=25.0, r_min_km=6.0, jerk_cap=1000.0),
    # hgv spd_min low: the modelled boost phase starts subsonic before the hypersonic glide (a
    # simplification of booster separation). g_cap 15: aggressive dense-air terminal weave/jink (CAV-H
    # terminal pull-up).
    "hgv":        dict(alt_max_km=130.0,  spd_min=500.0, spd_max=8000.0, g_cap=15.0, r_min_km=12.0, jerk_cap=600.0),
    # supersonic_cruise models a hypersonic cruise missile (Zircon/HACM-class scramjet): boost off the pad,
    # a ~24-30 km cruise at ~Mach 5-7.5, then a bounded terminal S-weave and dive; hence alt 32 km and
    # 2600 m/s. g_cap 16: terminal weave ~10-15 g. spd_min low: the boost starts subsonic off the rail.
    "supersonic": dict(alt_max_km=32.0,   spd_min=150.0, spd_max=2600.0, g_cap=16.0, r_min_km=3.0),
    # mirv: unpowered bus/RVs/decoys (gravity and drag only, no lift). Path curvature comes from drag and
    # gravity with no commanded turn, and reentry deceleration produces large physical jerk, so the
    # maneuver caps are loose. Trajectories are physical by construction (RK4); these limits are a sanity check.
    ...
    # decelerates far below an RV on reentry (the drag discriminant), and a chaff cloud can drift
    # near-subsonic. Same unpowered gravity-and-drag physics as mirv.
...
def audit_trajectory(pts: np.ndarray, dt: float, missile_class: str) -> Tuple[bool, Dict[str, tuple]]:
    ...
    # g_cap is a constant with no dynamic-pressure term, so this check does not test aerodynamic
    # feasibility. For example, a MaRV pulling 18 g at 57 km (qbar ~2.7 kPa, where the modelled airframe
    # can make ~0.1 g) passes the 25 g cap. `lat` is kinematic acceleration with gravity not removed, so on
    # a near-horizontal flier it differs by ~1 g from the lift the airframe must generate.
...
def audit_geometry_diversity(median_aspects_deg, min_std_deg: float = 25.0) -> Tuple[bool, str]:
    """Check that the dataset spans diverse threat-approach geometries. Fails if the standard
    deviation of per-scenario median aspect angle is below min_std_deg (deg). Returns (ok, detail)."""
...
if __name__ == "__main__":
    ...
    print("=" * 70)
    print("Physical-envelope checks on the force-integrated generators")
    print("=" * 70)
    all_pass = True
    ...
    print("\n" + ("All trajectories pass the physical-envelope checks." if all_pass
                  else "Some trajectories fail the physical-envelope checks; inspect them before adding to the dataset."))
