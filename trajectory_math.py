# trajectory_math.py
# Pure-Python trajectory math shared by the Blender script (main_sim.py) and the
# standalone scripts (experiments, dataset generation, ML). It has no bpy dependency
# and imports under any Python interpreter.
#
# Coordinate frame: WGS84 ellipsoid, geodetic latitude, the same frame as sim/radars.py,
# interceptor.py and sim/mirv.py.
import math
import numpy as np

# Spherical mean radius, used for the Blender globe size and coarse altitude references.
# Trajectory positions use the WGS84 conversion below.
R_EARTH_M = 6371e3
...
# Default scenario parameters (same values as main_sim.py)
...
def geodetic_to_ecef(lat_deg, lon_deg, alt_m=0.0):
    """Convert geodetic (WGS84) lat/lon (deg) and alt (m) to an ECEF position in metres.
    interceptor.py and sim/mirv.py import this conversion."""
...
# Alias imported under this name by main_sim.py and the trajectory generators.
...
def great_circle_points(a, b, n_points):
    ...
    interpolated linearly between |a| and |b|, so both endpoints are reproduced exactly
    on the WGS84 surface. Returns a list of n_points (3,) arrays.
    """
...
def build_trajectory(launch_lat=LAUNCH_LAT, launch_lon=LAUNCH_LON,
    ...
    time step (s). Uses the same math as the Blender script.
    """
