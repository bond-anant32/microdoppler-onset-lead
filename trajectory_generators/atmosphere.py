"""
trajectory_generators/atmosphere.py - US Standard Atmosphere 1976 (layered). Table values
follow the US Standard Atmosphere table distributed with MAPLEAF.

Density is log-interpolated in altitude (smooth, positive). Above the table top (86 km) an
exponential tail with a 6 km scale height is used, where density is negligible. Also exposes
temperature and sound speed (for Mach).
"""
from __future__ import annotations
import numpy as np

# US Standard Atmosphere 1976: altitude (m), density (kg/m^3), temperature (K).
_H = np.array([
