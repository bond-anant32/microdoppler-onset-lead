"""
trajectory_generators/corridors.py - fixed threat corridors with land-only sensor lay-downs.

Each corridor defines a launch region and a defended region on real land, the allowed missile
classes, and a plausible land-only sensor lay-down. Trajectories run launch -> defended region, and
sensors are placed at land sites that observe the corridor, so custody handover from the
launch-side network to the defended-side network follows from the geometry: a descending target
drops below the launch-side horizon into the defended-side radars' line of sight (the
has_line_of_sight() test in sim/measurements.py).

Land is tested with `global_land_mask.globe.is_land` (DC/Moscow/Tehran -> land,
mid-Atlantic/Pacific -> ocean). A sensor site over ocean is moved to the nearest land; if none is
within reach (a deep-ocean glide leg), no ground sensor is placed there, and the satellite layer
(sim/satellites.py) covers that over-horizon gap.

Sensor tiers (each tagged with band and frequency):
  land_x  : X-band fire-control (precise, short-medium range, fast update)
  land_s  : S-band acquisition
  vhf_ew  : VHF Voronezh/Nebo-class early-warning (coarse, very long range, slow update; anti-stealth)
"""
    ...
    def sample_endpoints(self, rng) -> Tuple[Tuple[float, float], Tuple[float, float]]:
        ...
        def _samp(center):
            for _ in range(50):
                ...
                if is_land_or_site(la, lo):
                    return (la, lo)
            return center                          # corridor centre (a hand-chosen land location)
        return _samp(self.launch_center), _samp(self.target_center)

    def sensors(self, rng, launch_ll, target_ll) -> List[Dict]:
        """Place a plausible land-only sensor lay-down for this corridor: a VHF EW site deep behind
        the launch (anti-stealth cue), X-band fire-control near the launch and around the defended
        asset, and S-band acquisition on a flank of the asset. Sites over ocean are moved to land or
        dropped. Returns a list of sensor dicts from make_sensor."""
        ...
        # Launch-side X-band fire-control (boost/midcourse acquisition). Flank angle and range are jittered
        # per site so the viewing aspect varies. Sites stand 120-260 km from the launch point.
        ...
        # Defended-side X-band fire-control and S-band acquisition form a ring around the asset, 60-200 km
        ...
        # S-band on a flank of the asset, 70 deg off the threat approach axis (brg+180), so no sensor sits
        # under the impact path.
        sc = dest_point(*target_ll, (brg + 180.0 + 70.0) % 360.0, float(rng.uniform(60.0, 110.0)))
        _add("land_s", sc[0], sc[1], "TC")
        return out


# Fixed corridors over real land regions. Ocean-crossing corridors are flagged transoceanic so a
# satellite is phased over their glide leg.
CORRIDORS: List[Corridor] = [
    ...
    # land on target and defended-side radars keep custody). The regional C1/C2 corridors are too short for a MIRV.
    Corridor("C7_transcon_mirv", (47.0, 80.0),  (49.0, 45.0),  3.0, ("mirv", "ballistic", "marv", "hgv", "supersonic_cruise")),
    # intercontinental land shots (~3500-4000 km), limited to classes with that range: ballistic/MIRV (ICBM),
    # boost-glide HGV, hypersonic cruise. A MaRV is a medium-range sub-ballistic RV; on a 3800 km shot it flies
    # near-ballistic (apogee ~950 km, no longer discriminably depressed), so it is limited to the <=2600 km
    # corridors (C1/C2/C7), where its depressed maneuvering arc is physical.
...
def random_long_corridor(rng, classes, min_km: float = 1900.0, max_km: float = 3100.0):
    """A random long land-to-land corridor, with both endpoints on real land min_km..max_km apart at a random
    location and azimuth. Long corridors spread scenes geographically and give a large shoot-look-shoot window.
    Sensors come from Corridor.sensors and are land-only. Tries up to 600 draws, then returns a fixed long
    fallback corridor."""
    for _ in range(600):
