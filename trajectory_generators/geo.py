...
# Geographic helpers for globally distributed scenarios and for placing a radar
# network along each trajectory, so that long-range targets remain observed.
import math
import numpy as np
...
def dest_point(lat_deg, lon_deg, bearing_deg, dist_km, R_km=6371.0):
    ...
    Used for choosing scenario endpoints; trajectory truth uses WGS84."""
...
def radar_network_for_trajectory(P, rng=None, power_db=100.0, layout=None):
    ...
    dataset spans a range of threat-approach geometries:
      overhead - along the ground track;                          side   - close broadside (~90 deg);
    ...
    for r in radars:                                         # randomize per-radar noise
