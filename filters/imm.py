...
A maneuvering-target tracker that runs three motion models in parallel and blends them by
how well each explains the radar measurements. A low-noise model dominates in cruise and a
higher-noise or turning model during a maneuver, with the switch governed by the mode
probabilities.

State (common to all modes, so mixing is dimensionally consistent):
...
Interface (same harness as filters/ekf.EKF):
    IMM_EKF(x0_9, P0_9, seed=...); .predict(dt); .update_radar(z, R, radar_ecef) -> NIS
    .update_angle(z_azel, R2, sensor_ecef) -> NIS
...
The measurement model is filters/ekf.h_and_H_radar (nonlinear radar model and Jacobian),
shared with the EKF baseline; only the motion models differ.

The CT turn angle per step is clamped, so a noisy acceleration estimate cannot produce an
impulsive turn. All models are purely kinematic (no gravity term); unmodeled gravity is
absorbed by process noise, as in the CV-EKF.
"""
...
def g_ct9(x: np.ndarray, dt: float, max_turn_rad: float = np.pi / 2) -> np.ndarray:
    ...
    # Co-rotate the acceleration with the velocity. In a coordinated turn the centripetal acceleration
    # stays perpendicular to velocity and rotates at the same omega; an unrotated `a` drifts out of
    # alignment with v_h, decays the inferred turn rate and adds a tangential (speed-changing)
    # component, which inflates NIS in sustained turns. The rotation is the identity when omega ~= 0.
    # Rotation about `up` leaves the vertical acceleration component unchanged.
...
def _num_jacobian(f, x: np.ndarray, dt: float, eps_scale: float = 1e-4) -> np.ndarray:
    """Forward-difference Jacobian of f(x, dt) at x, used for the nonlinear CT model."""
    n = len(x)
    fx = f(x, dt)
    J = np.zeros((n, n))
    for i in range(n):
...
def gaussian_likelihood(y: np.ndarray, S: np.ndarray):
    """Return (likelihood Lambda = N(y; 0, S), NIS = y^T S^-1 y).

    S is symmetrized and the NIS quadratic form is clamped non-negative, so a slightly non-PD or
    ill-conditioned S cannot yield a negative NIS, an overflowing exp and NaN mode probabilities.
    The log-likelihood is also capped at 700 before exponentiation."""
...
class IMM_EKF:
    ...
    def update_radar(self, z_meas, R, radar_ecef):
        ...
        for j in range(self.M):
            ...
            y = z_meas - z_pred
            y[1] = wrap_angle(y[1])         # wrap az/el only; range and Doppler are not angles
            y[2] = wrap_angle(y[2])
            S = H @ P @ H.T + R
        ...
        if np.isfinite(s) and s > 0 and np.all(np.isfinite(self._Lj)):
            self.mu = self._Lj / s      # otherwise keep the prior mu, so a degenerate likelihood cannot
            #                             propagate NaN into x and P through _combine
        self._combine()
        return float(self.mu @ nis_modes)   # mode-probability-weighted NIS

    def update_angle(self, z_azel, R2, sensor_ecef):
        """Angle-only (az, el) update from a passive IR sensor (IRST / LEO satellite).

        Uses a dedicated 2-vector update for numerical stability; padding range and Doppler with
        ~1e18 variance in the 4-vector update is ill-conditioned. One angle-only sensor constrains
        the transverse (cross-LOS) state; depth needs a second angle sensor or a ranging radar.
        Returns the mode-probability-weighted NIS."""
