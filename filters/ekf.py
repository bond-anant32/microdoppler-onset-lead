# filters/ekf.py
# Constant-velocity EKF on the 6-state ECEF [p, v], with radar (range, az, el, Doppler) and
# angle-only (az, el) measurement updates.
import numpy as np
...
def h_and_H_radar(x, radar_ecef):
    ...
    H[2,2] = r_xy/denom
    # Doppler partials. dop = v.dhat depends on velocity and on position through dhat = (p - radar)/r:
...
def h_and_H_angle(x, sensor_ecef):
    """Angle-only (az, el) measurement and 2x6 Jacobian for passive IR, IRST and satellite sensors.

    Returns the az/el rows of the radar model. The dedicated 2-vector update keeps the innovation
...
class EKF:
    ...
    def update_radar(self, z_meas, R, radar_ecef):
        ...
        y = z_meas - z_pred
        # wrap only the angular innovations (az, el) into [-pi, pi]; range and Doppler
        # residuals are left unwrapped
        y[1] = wrap_angle(y[1])
        y[2] = wrap_angle(y[2])
        S = H @ self.P @ H.T + R
