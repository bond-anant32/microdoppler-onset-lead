"""
sim/detection.py - probability of detection for Swerling target models.

Converts the SNR computed by the measurement model into a stochastic detection decision, so
the EKF/IMM sees missed detections (predict-only steps).

Standard radar detection theory (Mahafza, "Radar Systems Analysis and Design Using MATLAB";
...
fluctuation model. The code implements those published equations independently of the GPL
RadarSimPy source. Running the module as a script compares it against reference values from
RadarSimPy's test suite (tests/test_module_tools.py).

Swerling 1 (slow, scan-to-scan fluctuation) is the default, suited to a maneuvering target
whose aspect changes between scans.
"""
...
if __name__ == "__main__":
    # Compare against reference values from RadarSimPy's test suite.
    ...
    print("Detection formulas vs RadarSimPy reference values:")
    ok = True
    ...
    print("\n" + ("ALL PASS - matches RadarSimPy reference values." if ok
