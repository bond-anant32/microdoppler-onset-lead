"""
sim/sixdof.py - 6-DOF rigid-body missile airframe with an acceleration autopilot.

The interceptor's guidance lag T_guid and maximum divert a_max are outputs of this model and depend on
altitude, Mach and airframe geometry. Frame and quaternion conventions follow the open-source 6-DOF
flight simulator MAPLEAF:

  Frames: position and velocity in the global inertial frame; attitude is a unit quaternion, scalar-first
    [q0,q1,q2,q3], rotating global->body; body angular rate omega=(p,q,r) in the body frame; body +Z points
    to the tail (nose = -Z). Euler's rigid-body equations are integrated in the body frame and the
    quaternion is renormalized every step (RK4 drifts |q| at O(dt^4)).
  Pitch channel: a statically stable short-period airframe (Cm_alpha<0) driven by a 2nd-order fin servo
    under a rate+acceleration autopilot. The closed-loop step response has an equivalent time constant T
    (the guidance lag), and the trimmed lateral acceleration saturates at a_max = qbar*Sref*CN_max/m,
    which falls with altitude as dynamic pressure qbar=0.5*rho*V^2 drops. With DEFAULT_AIRFRAME,
    T ~ 0.2 s, a_max ~ 31 g at 10 km/M3 and AoA_max = CN_max/CN_alpha ~ 20 deg follow from one geometry.

Model limits: rigid body with no aeroelastic bending, so T is a lower bound; quasi-steady aerodynamics with
    CN clipped at CN_max (no unsteady aerodynamics, dynamic stall or transonic effects); no thrust-vector
    control and no mass, CG or inertia variation, so the model is a coasting terminal airframe whose a_max
    goes to zero outside the atmosphere (a divert-jet stage keeps a density-independent a_max); actuator
    limited by rate and deflection only (no backlash or hinge-moment stall). The seeker and estimator are
    not modeled here.
...
# Representative tactical-interceptor airframe (SM-class terminal stage). d=body diameter (m), Sref=frontal
# area (m^2), m=mass (kg), Iyy=pitch MOI, Ixx=roll MOI (kg m^2). Aero coefficients per radian. Values are
# representative surrogates after Zarchan, Fleeman and Nielsen, chosen so a_max at 10 km/M3 ~ 31 g,
# AoA_max ~ 20 deg and T ~ 0.2 s (a_max=qbar*Sref*CN_max/m, AoA_max=CN_max/CN_alpha,
# T ~ 2*zeta_cl/wn_cl + servo lag).
#
# The airframe's T is the resolved form of the lumped target time constant tau_T of the maneuvering-target literature.
# _validate() below checks it at 10 km, M3; experiments/onset_model_equivalence.py evaluates it at the
# 30 flight conditions used in the paper and gives T_63 = 0.179 s [0.176, 0.200], 11% from the lumped
# tau_T = 0.2 s of Fan et al. (EURASIP J. Adv. Signal Process., 2016), which has the first-order form
# of maneuvering.py's LAG_MODE=lag. The paper therefore commands a step: the step is the lumped model's
# input, and this airframe supplies the lag that the lumped model places in a single pole. The two
# differ at onset. The lumped lag moves at t=0+ with slope A/tau and crosses the 2.0 m/s^2 departure
# threshold in 12.1 ms; this airframe takes 48.45 ms (4.00x longer), because the delay is set by the
# servo, the short-period mode and the rate limit. Driving this airframe with a lagged command counts
# tau_T twice (2.09x on T_63, 2.35x on the onset delay).
...
# ----------------------------------------------------------------------------------------------------
# Full 6-DOF rigid-body derivative + RK4 (used for the attitude/EOM checks in _validate; the endgame uses the
...
class PitchAirframe:
    """Stateful planar (pitch-channel) reduction of the 6-DOF airframe with an acceleration autopilot, for use
    in place of a lumped first-order guidance lag. Call step(a_cmd, V, alt, dt) each guidance frame; it
    returns the lateral acceleration achieved through the airframe and servo, computed from the command
    alone (non-clairvoyant). T_guid and a_max are outputs of the model.
    ...
    def step(self, a_cmd, V, alt, dt):
        ...
        for _ in range(self.n_sub):
            ...
            self.alpha = float(np.clip(self.alpha, -aoa_max, aoa_max))   # AoA limiter, as in a real
            #   autopilot. Without it an over-command drives AoA past the stall angle (24 deg against
            #   the 20 deg limit), where CN is already saturated and adds no lift.
        return float(np.clip(self.a_z, -a_max, a_max))


# ----------------------------------------------------------------------------------------------------
# Measurement helpers (a_max, AoA_max and the step-response time constant)
# ----------------------------------------------------------------------------------------------------
def a_max_at(alt, mach, airframe=DEFAULT_AIRFRAME):
    """Maximum lateral divert (m/s^2) = qbar*Sref*CN_max/m at (alt, Mach)."""
...
def equivalent_time_constant(alt, mach, a_cmd_g=5.0, airframe=DEFAULT_AIRFRAME, dt=0.001, t_max=2.0):
    """Measure the guidance lag from a step response. Commands a step lateral acceleration of a_cmd_g
    (kept below a_max so the response is linear), logs achieved a_z(t) and returns
    (T_63, dc_gain, rise_10_90). T_63 is the time to reach 63.2% of the steady-state a_z (mean of the
    last 0.2 s), dc_gain is steady-state a_z over the command, and rise_10_90 is the 10-90% rise time."""
    V = mach * sound_speed(max(alt, 0.0))
...
def _validate():
    ...
    # (1) Guidance lag: the closed-loop step-response time constant lies in Zarchan's tactical homing
    #     band and near the lumped T_guid=0.2 s; DC gain is unity.
    ...
    # (2) a_max falls strictly with altitude as dynamic pressure drops, with anchors ~118 g SL,
    #     ~31 g @10 km, ~6.5 g @20 km and ~1.4 g @30 km. An aerodynamic interceptor has little divert
    #     at the altitudes where exo-atmospheric and HGV threats fly.
    ...
    # ratio law: the a_max ratio between altitudes equals the dynamic-pressure ratio
    ...
    # (3) AoA / CN saturation: a 60 g command saturates at a_max (~31 g), and the trimmed AoA stays at
    #     AoA_max = CN_max/CN_alpha (~20 deg).
    afr = PitchAirframe()
    ...
    # (4) Single-geometry consistency: AoA_max = CN_max/CN_alpha, and a_max = qbar*Sref*CN_max/m is
    #     evaluated at that AoA from the same coefficients.
    aoa = aoa_max_deg()
    ...
    # (5) Attitude / quaternion integration: a torque-free body with unequal principal moments
    #     (Ixx=2, Iyy=Izz=262 kg m^2) spinning about all three axes conserves global angular momentum and
    #     rotational kinetic energy over 30 s of RK4, and |q| stays at 1.
    ...
    # (6) Rotation sense: a pure +Y (pitch-up) body moment gives positive pitch rate q and swings the nose
    #     (-Z body) toward -X global, per the right-hand rule. A transposed or conjugated rotation fails this.
    ...
    print("=" * 82)
    print("6-DOF airframe and acceleration autopilot self-test (sim/sixdof.py)")
    print("=" * 82)
    ok = True
    for name, passed, detail in checks:
        ok = ok and passed
        print(f"  {'ok ' if passed else 'XX '} {name:62s} {detail}")
    print("\n" + ("ALL PASS - guidance lag T from the measured step response, divert a_max from"
                  "\n           qbar*Sref*CN_max/m, a_max falls with altitude, AoA saturates at"
