"""
sim/signatures.py - multi-modal radar signature synthesizer.

HRR, micro-Doppler and RCS are generated from one physical model: a per-class set of scattering
centers with micro-motions, driven by continuous physical state (aspect, Mach, lateral
acceleration, and a control-surface deflection cue that leads the kinematic turn). No output is
gated on the class label.

External tools under impfolder/:
  * OptixRCS (impfolder/3): aspect-resolved X-35 RCS maps calibrate the flash/null dynamic range;
    the trihedral reflector is the analytic RCS check.
  * RadarSimPy (impfolder/1): its range_fft / doppler_fft DSP synthesize the HRR (range
    compression) and micro-Doppler (STFT) from the scattering-center returns. The DSP modules are
    loaded through a shim that skips the compiled C++ engine; a NumPy Hann-windowed FFT is used
    if the import fails.
  * Fusion-DeepONet (impfolder/4): its bow-shock pressure field sets the HGV plasma-sheath effect
    (deterministic RCS bias and Doppler broadening at high Mach and low altitude).
  * MAPLEAF (impfolder/2): source of the layered atmosphere in trajectory_generators/atmosphere
    and of the NASA 6-DOF attitude data used for EKF/IMM validation.

Anticipation mechanism: a maneuver is preceded by control-surface deflection. The generator
applies the recorded command with delay trajectory_generators.maneuvering.ACTUATOR_LAG_S = 1.2 s,
and the micro-Doppler of the control-surface scatterer tracks the command, so the signature
carries a cue that precedes the kinematic turn.
"""
...
LEAD_S = 1.5              # unused in this module; the command-to-kinematic lag is
                          # maneuvering.ACTUATOR_LAG_S = 1.2 s

# ---------------------------------------------------------------------------
# Tool 1: RadarSimPy DSP (range_fft / doppler_fft), loaded without its compiled engine.
# ---------------------------------------------------------------------------
...
def _range_fft(fast_time: np.ndarray) -> np.ndarray:
    """Range compression of a fast-time return -> HRR (magnitude). Uses RadarSimPy's
...
def _doppler_fft(slow_time_window: np.ndarray) -> np.ndarray:
    """One Doppler spectrum of a slow-time window. Uses RadarSimPy's doppler_fft
...
def _load_x35_dynamic_range() -> float:
    """Broadside flash (99th percentile) minus median (dB) of the X-35 aspect map, used to scale
    the synthetic aspect dependence to an observed dynamic range. Falls back to 30 dB (typical
    airframe flash/null span) if the file is missing or has fewer than 100 values."""
...
def _load_plasma_pressure_ratio() -> float:
    ...
    clipped to [10, 500]. Sets the magnitude of the plasma-sheath effect (RCS bias and Doppler
    broadening) applied to HGV signatures at high Mach and low altitude. Falls back to 130 if
...
#   shape, dims  : canonical analog and its dimensions (m)
#   micro        : micro-motion role: 'tip' (aspect-broad, small), 'cavity' (corner reflector:
#                  broad, nose-biased), 'cylinder' (broadside specular flash), 'fin'/'control'
#                  (small, micro-motion)
# RCS anchors: DF-15 head-on ~-17 dBsm; the ramjet intake cavity raises the nose-on return; the
# cylinder broadside flash is far above nose-on. Calibrated against the OptixRCS X-35 dynamic range.
# ---------------------------------------------------------------------------
# --- Canonical-analog physical-optics RCS --------------------
# Following defense practice (DTIC/AFIT/IEEE Aerospace CEM), a target whose exact RCS is classified
# is modelled as a set of canonical geometric analogs with analytic physical-optics RCS. Each
# scattering center is one canonical shape with real dimensions, and its RCS comes from the
# textbook PO formula at the operating wavelength with specular lobing (sinc^2). po_rcs_dbsm()
# reproduces the impfolder anchors: sphere -1.05 dBsm, plate 48.3/59.2 dBsm, trihedral ~32 dBsm.
#   theta convention (po_rcs_lin): 0 = the analog's boresight along the body long axis, pi/2 = broadside.
#   Plate and cylinder lobes sinc^2(k*L*cos t) and the cone_tip term |sin t| are symmetric under t -> pi-t;
#   the trihedral lobe is not. The caller's aspect (measurements._aspect_angle_rad) uses pi = nose-on,
#   and _center_rcs_lin maps between the two conventions.
def po_rcs_lin(shape: str, dims: Dict[str, float], theta: float, lam: float) -> float:
    ...
    Pure specular PO with no diffuse floor, so it reproduces the exact anchors; the small structural
...
def _diffuse_lin(shape: str, dims: Dict[str, float]) -> float:
    """Small absolute structural/diffuse RCS (m^2), independent of the narrow specular maximum.
    It holds off-flash aspects at a realistic low level (~-20..-8 dBsm)."""
    if shape == "cylinder":
...
def po_rcs_dbsm(shape: str, dims: Dict[str, float], theta: float = 0.0,
    ...
    """Canonical-analog PO RCS in dBsm at carrier frequency freq_hz."""
...
SCATTERERS: Dict[str, List[Center]] = {
    ...
    "decoy": [               # light decoy: matches the RV's RCS; drag discriminates it
        Center(0.0, "cone_tip", {"sigma": 1.2e-2}, "tip"),
        Center(0.9, "cylinder", {"r": 0.28, "L": 1.2}, "fin")],
    # --- decoy types with different physics, spanning a range of discrimination difficulty. The
    #     ballistic coefficient beta is set in sim/mirv.py; these entries set the radar shape. ---
    "decoy_replica": [       # hard: faithful RV replica with matched RCS shape and near-RV beta; only a
        Center(0.0, "cone_tip", {"sigma": 1.2e-2}, "tip"),           # small micro-motion difference separates it
        Center(1.0, "cylinder", {"r": 0.28, "L": 1.4}, "cylinder")],
...
MICRO_HZ: Dict[str, Dict[str, float]] = {
    ...
    "hgv":               {"roll": 1.2},                      # bank-to-turn roll
    ...
    "decoy_replica":     {"spin": 2.0, "precession": 0.55},  # hard: RV spin with slightly offset precession
    "decoy_balloon":     {"tumble": 0.6},                    # medium: large light object, slow tumble
...
def _effective_aspect(aspect_rad: float, aoa_rad: float) -> float:
    ...
    Returns |aspect - aoa|, capped at pi: when the AoA rotates the body axis past the LOS
    (aoa > aspect near nose-on), the LOS lies on the other side of the body axis at aoa - aspect."""
...
def _center_rcs_lin(ctr: "Center", aspect_rad: float) -> float:
    ...
    Aspect convention: the caller's aspect (sim/measurements._aspect_angle_rad, LOS built as
    radar->target) is pi when the missile flies toward the radar (nose-on) and 0 when it recedes
    (tail-on). po_rcs_lin uses 0 = the analog's boresight (trihedral ~32 dBsm at theta = 0). The
    trihedral is the only shape whose lobe is asymmetric over [0, pi], so its aspect is mapped to
    pi - aspect; the other shapes are symmetric under t -> pi-t and take the aspect unchanged. With
    this mapping the supersonic ramjet engine-inlet cavity (a forward-facing retroreflector at
    down_range 2.0 m on an ~8 m body) flashes nose-on."""
...
def rcs_dbsm(missile_type: str, aspect_rad: float, alt_m_true: float, mach: float,
    ...
    """Aspect-summed RCS (dBsm) from the canonical-analog scattering centers (analytic PO with
    specular lobing), using the true geodetic altitude. Adds sea-skim multipath,
    control-deflection lift, and (HGV) plasma-sheath bias."""
    centers = SCATTERERS.get(missile_type, SCATTERERS["supersonic_cruise"])
    control = control or Control()
    eff = _effective_aspect(aspect_rad, control.aoa_rad)   # body axis tilted by AoA
    ...
    if alt_m_true < 100.0:
        ...
        base -= 1.0
    # Exposed control surfaces raise RCS continuously with the deflection cue.
...
def hrr_profile(missile_type: str, aspect_rad: float, mach: float,
    ...
    so peak spacing is a continuous function of aspect."""
    centers = SCATTERERS.get(missile_type, SCATTERERS["supersonic_cruise"])
    control = control or Control()
    rng = rng or np.random.default_rng()
    eff = _effective_aspect(aspect_rad, control.aoa_rad)   # AoA-tilted body aspect
    n_freq = n_bins
...
def micro_doppler(missile_type: str, aspect_rad: float, mach: float, flight_state: str,
    ...
    for ctr in centers:
        ...
        # Body-rate term: as the airframe pitches to develop the commanded AoA (ahead of the
        # kinematic turn by the actuator lag), every scattering center receives a common pitch-rate
        # micro-Doppler scaled by the command cue, so the cue appears on the bright body returns
        # as well as on the fins.
        ...
        if missile_type == "marv" and ctr.micro == "control":
            ...
            if ctr.micro == "control":
                # control-surface flutter whose amplitude tracks the deflection cue
...
def control_series_from_trajectory(P: np.ndarray, V: np.ndarray, dt: float,
    ...
    """Per-step Control drivers. The anticipation cue (`lead_cue`) is driven by the commanded
    lateral acceleration `cmd_lat_accel`, the control input recorded by the generator. The
    trajectory applies that command through an actuator lag, so the cue precedes the kinematic
    turn. If no command is supplied, the cue falls back to the
    kinematic lateral-acceleration envelope and carries zero lead. `aoa_rad` tilts the body axis
    off the velocity vector in proportion to lateral load."""
    n = len(P)
    ...
    k = max(3, int(smooth_s / dt))                      # cue-filter length in samples; a shorter filter delays the cue less
    kernel = np.ones(k) / k
    # Cue source: the command if available, else the kinematic lateral acceleration.
    src = np.abs(np.asarray(cmd_lat_accel, dtype=float)) if cmd_lat_accel is not None else lat
    if len(src) != n:                                   # length mismatch: use the kinematic source
        src = lat
    ref = max(np.percentile(src, 95), 1.0)
    # Causal trailing smoothing: mode="full"[:n] is a backward-only moving average, so the cue at
    # sample i depends only on src[:i+1]. A centred window would reach ~0.5 s ahead and, on the
    # kinematic fallback branch, feed the future maneuver into the current feature. On the command
    # branch the command leads the kinematic turn by ACTUATOR_LAG_S.
    cue = np.clip(np.convolve(np.clip(src / ref, 0.0, 1.0), kernel, mode="full")[:n], 0.0, 1.0)

    # AoA proxy: the body pitches to generate the commanded lift; AoA ~ lateral load / (C_Lα q).
    # Bounded map of the kinematic lateral load to a plausible AoA (deg), capped at 22 deg.
    g = 9.80665
