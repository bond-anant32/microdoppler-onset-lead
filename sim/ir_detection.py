"""
sim/ir_detection.py - space-IR radiometry and probabilistic detection, the IR counterpart of
sim/detection.py.

Physics (first-principles and open-source surrogates; no classified data; not a NEQAIR/SIRRM model):
  * Radiant intensity = max(boost plume, aerothermal skin), each a Planck band integral over MWIR
    (Lambertian I = A * M_band(T) / pi). The exhaust plume is a T = 2500 K blackbody
    (solid-rocket-motor combustion, Planck's law) over a class exhaust area, scaled by an OSINT
    burn profile (full through the ~60 s boost burn, dropping at burnout). This is the launch
    signature a DSP/SBIRS-class sensor detects. The 2500 K blackbody over the class area recovers
    the OTA (1988) hierarchy (ICBM ~1e6, theater ~1e5 W/sr). The skin term (recovery-temperature
    blackbody, capped at a radiative wall temperature) is the HGV-glide/reentry tracking signal.
  * Atmospheric transmittance exp(-k*airmass), k ~ 0.12/km MWIR (MODTRAN-class band figure). Only
    the ~0-20 km layer attenuates (loss-free above), and airmass grows as 1/cos(zenith), so the
    plume brightens to the satellite as it climbs out of the dense atmosphere.
...
The model is checked for self-consistency with detection theory: __main__ cross-checks the NP
threshold against the documented npwgnthresh value. sim/detection.py is checked against RadarSimPy;
no open source gives IR Pd at specific SNRs, so this module has no external Pd reference. NEI and
the emitting areas are calibration parameters anchored to published detectability thresholds.
"""
...
#                              # grows as M^2, and hypersonic walls sit at ~1100-2600 K.
T_PLUME_K = 2500.0            # exhaust-plume blackbody temperature (K), solid-rocket-motor combustion,
#                              # an open-source value modeled via Planck's law
BURN_TIME_S = 60.0           # nominal SRBM boost burn (typ. 60-90 s); OSINT burn profile
NEI_WM2 = 5e-10              # noise-equivalent irradiance, cooled MWIR FPA (calibration parameter; places
#                              # DSP 20 kW/sr at ~+14 dB @1200 km, the published detectability anchor)
K_MWIR_PER_KM = 0.12         # MWIR band-avg extinction near surface (MODTRAN-class)
...
# Exhaust-plume cross-section area (m^2) by booster class; intensity = Planck(2500 K) band exitance
# * area / pi. Larger boosters have larger, brighter plumes (recovers the OTA 1988 hierarchy).
PLUME_AREA_M2 = {"ballistic": 20.0, "booster": 20.0, "mirv": 20.0, "hgv": 10.0,
                 "marv": 6.0, "supersonic_cruise": 3.0, "bus": 3.0, "rv": 0.5, "decoy": 0.5}
# body projected/emitting area (m^2) for the aerothermal skin term, consistent with sim/signatures SCATTERERS.
...
def planck_band_exitance(T, lam1=3e-6, lam2=5e-6, n=64):
    """MWIR (3-5 um) band radiant exitance M (W/m^2) of a blackbody at T, by trapezoidal integration
    over n wavelength samples of Planck's law M(lam,T) = 2*pi*h*c^2 / (lam^5 (exp(hc/lam k T) - 1))."""
...
def radiant_intensity_wsr(missile_type, flight_state, mach, alt_m, t_since_launch=None, burn_time=BURN_TIME_S):
    ...
    Planck band integral (Lambertian I = A * M_band(T) / pi):
      * plume: a T_PLUME_K (2500 K) blackbody over the class exhaust area, active during boost and
        scaled by the OSINT burn profile (bright for t = 0..burn_time, dropping at burnout). Altitude
        dependence enters through atmospheric_transmittance.
...
def ir_threshold(pfa, n_frames=1):
    ...
    # threshold continuous with the N == 1 closed form above (2*gammainccinv(1, Pfa) = -2 ln Pfa).
...
def ir_detected(snr_db, rng, pfa=1e-6, n_frames=1):
    ...
    tracker takes a predict-only step (as sim/detection.py does for radar). Returns (bool, Pd)."""
    pd = pd_ir(snr_db, pfa, n_frames)
    return bool(rng.random() < pd), pd


def fluctuate_snr_db(snr_db, rng, shape=4.0):
    """Per-look IR irradiance fluctuation (plume flicker / scintillation), gamma-distributed about the
    mean on linear irradiance. The fluctuated value is the reported SNR; the Pd draw uses the mean
    snr_db, as in the radar model. Returns a fluctuated snr_db."""
...
def _validate():
    ...
    checks = []
    # (1) Pd pins from the Marcum-Q closed form at Pfa=1e-6 (computed; no external source)
    ...
    # (2) Pd at 6 dB is ~1%, so a hard 6 dB threshold overstates detection
    checks.append(("Pd(6 dB) < 0.1", pd_ir(6.0) < 0.1,
    ...
    checks.append(("NP threshold reproduces npwgnthresh 7.3335 dB", abs(npw - 7.3335) < 1e-3,
                   f"{npw:.4f} dB"))
    # (4) launch detectability: a boost-phase plume is detected with the published margin
    #     (ICBM 6e5 W/sr at 5 km alt, R=1500 km, near-overhead); a cold subsonic body is not
    ...
    # (6) atmospheric extinction: a low grazing path attenuates more than a high overhead one
    ...
    # (8) Planck plume: a 2500 K blackbody over the exhaust area recovers the ICBM ~1e6 W/sr anchor
