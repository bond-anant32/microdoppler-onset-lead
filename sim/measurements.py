...
# ------------- Config: path to the main simulation module -------------
# Example: MAIN_PATH = "/absolute/path/to/main_sim.py"
MAIN_PATH = os.environ.get("SIM_MAIN_PATH", "").strip()
# ---------------------------------------------------------------

# Import a module from a file path (importlib recipe from the Python docs)
...
# Radar network (importable via PYTHONPATH or as a package)
from sim.radars import RADARS_ECEF, should_radar_emit
from sim import signatures as _sig               # scattering-center signature model
from sim.detection import detected                # Swerling Pd detection decision
...
# Aspect-dependent RCS surrogate: (nose_on_dBsm, broadside_dBsm), X-band nominal, calibrated to
# open-source physical-optics estimates. Sources: the DF-15 POFACETS estimate (~-17 dBsm head-on
# at X-band; Touzopoulos and Zikidis, JCM 7(1), 2017); Knott/Skolnik ch. 11 lobing physics (the
# cylinder broadside flash is far above nose-on, and an engine-inlet cavity acts as a re-entrant
# corner reflector that raises supersonic-cruise nose-on RCS); RAM ~-10 dB and VHF-resonance
# rules of thumb. All values are estimates.
RCS_DATABASE = {
...
def _simulate_snr_db(range_m: float, rcs_dbsm: float,
    ...
    RCS (dB), offset by radar power relative to 100 dB. A 0 dBsm target at ref_range_km seen
    by a nominal (power=100) radar gives snr_ref_db (20 dB). At this scale the Swerling Pd
    decision drops detections at long range and low RCS. Output is clipped to [-30, 60] dB.
    """
...
def _generate_clutter(radar: Dict, rng, lam: float,
    ...
    FOV (birds, weather, civil traffic). Each false alarm is a marginal detection with a
    near-threshold SNR (8-16 dB), the corresponding quality value, and noise-like HRR and
    micro-Doppler arrays of the same shape as a target return. is_clutter=True is the
    ground-truth label and is not one of the measurement features."""
    out: List[Dict] = []
    for _ in range(int(rng.poisson(lam))):
        ...
        snr = float(rng.uniform(8.0, 16.0))                 # marginal: just above the detection threshold
        # bird/weather-like signature: low-structure arrays
...
def has_line_of_sight(r_ecef, p_ecef, r_earth: float = 6371000.0) -> bool:
    """True if the straight radar->target segment does not pass below the Earth (horizon
    occlusion). Uses a spherical Earth of radius r_earth with a 50 m tolerance; the ~20 km
    ellipsoid difference is negligible for an occlusion test."""
...
def simulate_radar_measurements(
    ...
    Inputs:
      ...
      - meas_context: optional dict to drive advanced fields, e.g.:
        ...
        optional sub-configs: "clutter", "control", "detection", "swerling", "glint"
      - radars: radar list (default RADARS_ECEF)
      - glint_states: dict carrying the AR(1) glint state per radar between calls
...
def _detect_target(radar, p_ecef, v_ecef, ctx: Dict, glint_states, glint_key, rng):
    ...
    (missed detection or horizon occlusion). Shared by the single- and multi-object entry points."""
    R_true, az_true, el_true, dop_true = geom_to_measurement(p_ecef, v_ecef, radar["ecef_m"])
    if R_true <= 1e-6:                               # degenerate geometry
        return None
    if not has_line_of_sight(radar["ecef_m"], p_ecef):   # Earth occludes the target
        return None
    ...
    # Signature from the scattering-center model (sim/signatures.py) at the true altitude.
    ...
    # The Swerling Pd decision uses the mean SNR, since the closed forms already integrate the
    # fluctuation; the instantaneous fluctuated RCS and SNR are reported in the measurement.
...
def main_entry():
    ...
    # Import the main module from its file path
